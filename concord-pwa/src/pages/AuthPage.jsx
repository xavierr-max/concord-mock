...
export function AuthPage({ initialMode = 'login', navigate }) {
  ...
  return <main className="auth-page">
    <section className="auth-branding"><button className="brand-button" onClick={() => navigate('/')}><Brand /></button><div><span className="eyebrow">COMUNICAÇÃO, REIMAGINADA</span><h1>Onde ideias ganham <em>conexão.</em></h1><p>Converse, colabore e esteja perto da sua comunidade.</p></div></section>
    <section className="auth-panel"><div className="auth-card"><Brand compact /><h2>{mode === 'login' ? 'Boas-vindas de volta' : 'Crie sua conta'}</h2><p>{mode === 'login' ? 'Entre para continuar no Concord.' : 'Comece uma nova comunidade.'}</p>
      <div className="auth-tabs"><button className={mode === 'login' ? 'active' : ''} onClick={() => navigate('/login')}>Entrar</button><button className={mode === 'register' ? 'active' : ''} onClick={() => navigate('/register')}>Cadastrar</button></div>

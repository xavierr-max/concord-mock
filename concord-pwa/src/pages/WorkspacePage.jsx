...
export function WorkspacePage() {
  ...
  const notifications = useNotifications(showError)
  ...
  return <div className="workspace">
    ...
    <button className="notification-fab" aria-label={`${notifications.unreadCount} notificações não lidas`} onClick={() => setModal({ kind: 'notifications' })}>♢{notifications.unreadCount > 0 && <b>{notifications.unreadCount > 99 ? '99+' : notifications.unreadCount}</b>}</button>
    {modal && <WorkspaceModal modal={modal} server={server} channel={channel} profile={profile} user={user} isOwner={isOwner} canManage={canManage} notifications={notifications} onClose={() => setModal(null)} onSave={saveEntity} onConfirm={confirmDelete} onLogout={logout} setModal={setModal} />}
...
function WorkspaceModal({ modal, server, channel, profile, user, isOwner, canManage, notifications, onClose, onSave, onConfirm, onLogout, setModal }) {
  ...
  if (modal.kind === 'notifications') return <Modal title="Notificações" onClose={onClose} actions={notifications.unreadCount > 0 && <button className="secondary-button" onClick={() => notifications.markAllRead()}>Marcar todas como lidas</button>}><div className="notification-list">{notifications.items.length === 0 ? <p className="muted-copy">Nenhuma notificação por enquanto.</p> : notifications.items.map(item => <button key={item.id} className={item.isRead ? 'read' : ''} onClick={() => !item.isRead && notifications.markRead(item.id)}><i>{notificationIcon(item.type)}</i><span><strong>{item.title}</strong><p>{item.content}</p><small>{new Date(item.createdAt).toLocaleString('pt-BR')}</small></span>{!item.isRead && <b aria-label="Não lida" />}</button>)}</div></Modal>
...
function notificationIcon(type) { return ({ Mention: '@', NewMessage: '#', InviteCreated: '↗', InviteAccepted: '✓', ServerMemberJoined: '+' })[type] || '•' }
